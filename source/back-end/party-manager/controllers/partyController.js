...
exports.createParty = (req, res) => {
  ...
  accountDb.find(partyObj).then(result => {
    if (result !== null) {
      ...
      accountDb.create({ partyName: partyObj.partyName, userName: partyObj.hostName }).then(resp => {
        userDb.create(
          {
            partyId: resp._id,
            userName: partyObj.userName,
            favoredArtist: artistName,
            favoredGenere: genereName,
            favoredSong: songName
          })
          .then(resp => {
            res.send(resp);
          })
